...
export function buildJanamPatriHtml(d: JanamPatriData): string {
  ...
  const calcNote = `<div class="calcnote"><b>Calculation Basis:</b> Birth date, exact birth time, timezone and place are used for VedAstro-backed sidereal calculations. Ayanamsa shown by API/report: ${esc(ayanamsa)}. AI text only explains calculated Kundli, dasha and transit data; it is not used to invent planetary positions or dates.</div>`;
...
<section class="pdf-section cover">
  ...
  <section class="pdf-section compact-section"><p class="foot">Calculated via VedAstro (Lahiri Ayanamsa) + classical Jyotish · Generated by Shree Yantra App<br/>
